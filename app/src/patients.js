...
export {
  fromDate, toDate, datesRange, datesScale,
  encounters, encountersMap, relations,
  currentEncounters, currentTopics, numActiveEncounters, numActiveRelations,
  enc_eid, enc_date, enc_zipcode, enc_tags, enc_eid_det,
  topics_tid, topics_cat, topics_sys,
  rel_tid_cg, tag_enc_group,
  rel_tid,
  init, set, addDetector, update
}
...
/*export*/ let encountersMap = new Map();
...
function setup(data) {
  ...
  let from  = d3.timeDay.ceil(dateParse(fromDate)),
      to    = d3.timeDay.offset(d3.timeDay.ceil(dateParse(toDate)), 1);

  datesRange = d3.timeDay.range(from, to);
